...
function sendMessage(msg, response) {
    setTimeout(() => chrome.runtime.onMessage.emit(msg, null, response), 0)
}
...
function virtualKeyboardChromeExtension_click(key, skip) {
    if (top != self) {
        sendMessage({ method: "clickFromIframe", key: key, skip: skip, frame: this.frameElement.id });
    } else {
        if (key != "Close") {
...
function setting_load(key, callback) {
    sendMessage({ method: "getLocalStorage", key: key }, function (response) {
        eval(callback)(response.data);
    });
}

function setting_reloadKeyboardToggle(openState) {
    sendMessage({ method: "getLocalStorage", key: "keyboardEnabled" }, function (response) {
...
function setting_set(key, value) {
    sendMessage({ method: "setLocalStorage", key: key, value: value }, function (response) { });
}

function openUrl(url) {
    sendMessage({ method: "createTab", url: url }, function (response) { });
}
...
function virtualKeyboardChromeExtension_open_part2(pos) {
    ...
    sendMessage({ method: "getSmallKeyboardCoords" }, function (response) {
...
function virtualKeyboardChromeExtension_open(posY, posX, force) {
    if (top != self) {
        ...
        sendMessage({ method: "openFromIframe", posY: posY, posX: posX, force: force, frame: this.frameElement.id, elem: virtualKeyboardChromeExtensionClickedElem.id });
    } else {
        ...
        if (top == self) {
            if (virtualKeyboardChromeExtensionTouchEvents == undefined) {
                if ((document.getElementById('virtualKeyboardChromeExtension').getAttribute("_state") != "open") || (virtualKeyboardChromeExtensionRequestRefresh)) {
                    sendMessage({ method: "loadKeyboardSettings" }, xk_settings_load_main);
                }
            } else {
            ...
            if (virtualKeyboardChromeExtensionTouchEvents == undefined) {
                sendMessage({ method: "loadKeyboardSettings" }, function (response) {
                    virtualKeyboardChromeExtensionTouchEvents = response.touchEvents;
                    if (virtualKeyboardChromeExtensionTouchEvents == undefined) { virtualKeyboardChromeExtensionTouchEvents = "false"; }
                    init_virtualKeyboardChromeExtension(true);
                });
        ...
        if (firstTime) {
            ...
            if (top == self) {
                ...
                if (document.getElementById("settingsButton") !== null) {
                    sendMessage({ method: "getLocalStorage", key: "keyboardLayoutsList" }, function (response) {
...
function vk_ajax_load_main() {
    if (xhr.readyState == 4) {
        ...
        sendMessage({ method: "initLoadKeyboardSettings" }, function (response) {
